...
function processMessages() {
    ...
    isProcessing = true;
    try {
    	console.log('tryprocessMessages()')
        var msg = popMessageFromQueue();
        // The Java side can send a * message to indicate that it
        // still has messages waiting to be retrieved.
        if (msg == '*' && messagesFromNative.length === 0) {
            nextTick(pollOnce);
            console.log('pollOncetryprocessMessages()')
            return;
        }
        processMessage(msg);
    } finally {
...
exports.load = function (callback) {
    ...
    injectIfNecessary('cordova/plugin_list', pathPrefix + 'cordova_plugins4android.js', function () {
